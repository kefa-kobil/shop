/*
  ...
  1. New Tables
    - `pos_transactions` - Store POS transaction records
    - `pos_transaction_items` - Store individual items in each transaction
    - `pos_sessions` - Track cashier sessions and daily totals
  ...
  4. Sample Data
    - Create sample cashier user and profile
...
DO $$
DECLARE
  cashier_user_id uuid := '44444444-4444-4444-4444-444444444444';
BEGIN
  -- First, insert into users table if it exists
  ...
  INSERT INTO profiles (id, full_name, role, created_at, updated_at) VALUES 
    (cashier_user_id, 'Test Cashier', 'cashier', NOW(), NOW())
  ON CONFLICT (id) DO UPDATE SET
    full_name = EXCLUDED.full_name,
    role = EXCLUDED.role,
    updated_at = NOW();
