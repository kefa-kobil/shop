/*
  ...
  2. Security
    ...
    - Add policies for cashiers and managers
    - Cashiers can only access their own data
    - Managers can access all POS data
...
BEGIN
  -- Check if users table exists and if user already exists
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'users') THEN
    SELECT COUNT(*) INTO existing_user_count FROM users WHERE id = cashier_user_id;
