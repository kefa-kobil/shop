/*
  ...
  1. New Tables
    - `pos_transactions` - Store point of sale transactions
    - `pos_transaction_items` - Store individual items in each transaction
    - `pos_sessions` - Store cashier sessions for tracking daily sales

  2. Security
    - Enable RLS on all POS tables
    - Add policies for managers, admins, and cashiers
  ...
  4. Sample Data
    - Uses existing profiles from the database
    - Creates sample sessions and transactions for demo
*/
...
BEGIN
  ...
  IF manager_id IS NOT NULL AND array_length(product_ids, 1) > 0 THEN
    ...
    -- Insert sample POS session using existing manager as cashier
    INSERT INTO pos_sessions (cashier_id, register_number, opening_cash, status, opened_at) 
    VALUES (manager_id, 'REG-001', 200.00, 'open', now() - interval '2 hours')
